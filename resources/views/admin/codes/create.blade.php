    ...
    <div class="row g-4">
        <div class="col-sm-12">
            <div class="bg-secondary rounded h-100 p-4">
                <h5 class="mb-4">Create Code</h5>
                <form action="{{ route('codes.store') }}" method="post" id="codeForm">
                    @csrf
                    <div class="mb-3 position-relative">
                        <input type="text" class="form-control @error('category') is-invalid @enderror" id="category"
                            name="category" value="{{ old('category') }}" aria-describedby="category"
                    ...
                    <div class="mb-3">
                        <input type="text" class="form-control @error('title') is-invalid @enderror" id="title"
                            name="title" value="{{ old('title') }}" aria-describedby="title" placeholder="Title">
                        @error('title')
                        <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="form-floating">
                        <textarea class="form-control @error('code') is-invalid @enderror"
                            placeholder="Write a code here" id="code" style="height: 150px;"
                            name="code">{{ old('code') }}</textarea>
                        @error('code')
                        <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
